...
torch_register_pytree_node(slice, lambda s: ((s.start, s.stop, s.step), None),
...
def mk_rng() -> jax.random.PRNGKey:
    assert len(_RNG_POOPER_STACK
               ) > 0, "Attempted `mk_rng()` outside of a `RngPooperContext`"
...
class Torchish:
    ...
    def value(self, val):
        ...
        assert isinstance(val, jnp.ndarray) or isinstance(
            val, (np.ndarray, int, float)), (
                f"Tried to create Torchish with unsupported type: {type(val)}")
        self._value = val if torch.is_grad_enabled(
        ) else jax.lax.stop_gradient(val)
    ...
    def expand(self, *sizes):
        assert len(
            sizes) == self.ndim, "TODO: implement len(sizes) > self.ndim"
        newshape = [
            new if new != -1 else old for old, new in zip(self.shape, sizes)
        ]
    ...
    def view(self, *shape_or_dtype):
        if len(shape_or_dtype) == 1:
            if isinstance(shape_or_dtype[0], Sequence):
                ...
                raise ValueError(
                    f"Tensor.view takes shape or dtype, got {shape_or_dtype[0]}"
                )
    ...
    def bernoulli_(self, p=0.5):
        ...
        self.value = jax.random.bernoulli(mk_rng(), shape=self.shape,
                                          p=p).astype(self.value.dtype)
        return self

    def uniform_(self, a, b):
        self.value = jax.random.uniform(mk_rng(),
                                        shape=self.shape,
  ...
  JAX-compatible."""
    if isinstance(x, Torchish):
        return x.value
            ...
            x, (int, float, np.ndarray, jnp.ndarray)):  # jax compatible types
        return x
    elif any(x is e
             for e in (None, Ellipsis)):  # jax compatible special values
        return x
...
def _tree_coerce(x):
    ...
    while len(specs) > 0:
        ...
        jax.tree_util.register_pytree_node(
            ...
            node.flatten_fn,
            flip_args(
                node.unflatten_fn),  # torch & jax have different order of args
        )
...
def implements(torch_function,
    ...
    def decorator(func):
        if out_kwarg:
            ...
            func1 = lambda *args, **kwargs: torch_tree_map(
                Torchish, func(*args, **kwargs))
        else:
            func1 = func
...
def auto_implements(torch_function,
                    jax_function,
                    dont_coerce_argnums=(),
                    out_kwarg=False,
                    Torchish_member=False):
    ...
    @implements(torch_function,
                out_kwarg=out_kwarg,
                Torchish_member=Torchish_member)
    def fn(*args, **kwargs):
        # NOTE: we don't _coerce values in kwargs! So far this has not been problematic.
        return jax_function(
            *(arg if i in dont_coerce_argnums else _coerce(arg)
...
auto_implements(torch.bitwise_not,
                jnp.invert,
                out_kwarg=True,
                Torchish_member=True)
auto_implements(torch.cos, jnp.cos, out_kwarg=True, Torchish_member=True)
auto_implements(
    torch.clone, lambda x: x,
    Torchish_member=True)  # jax arrays are immutable, no copy needed
...
auto_implements(torch.logical_and,
                jnp.logical_and,
                out_kwarg=True,
                Torchish_member=True)
auto_implements(torch.logical_or,
                jnp.logical_or,
                out_kwarg=True,
                Torchish_member=True)
auto_implements(torch.logical_not,
                jnp.logical_not,
                out_kwarg=True,
                Torchish_member=True)
auto_implements(torch.logical_xor,
                jnp.logical_xor,
                out_kwarg=True,
                Torchish_member=True)
auto_implements(torch.mul, jnp.multiply, out_kwarg=True, Torchish_member=True)
auto_implements(torch.nan_to_num,
                jnp.nan_to_num,
                out_kwarg=True,
                Torchish_member=True)
# Tensor.permute has a different signature than torch.permute
auto_implements(torch.permute, jnp.transpose,
                dont_coerce_argnums=(1, 2))  # TODO: do we need argnum 2?
auto_implements(torch.pow, jnp.power, out_kwarg=True, Torchish_member=True)
auto_implements(torch.rsqrt,
                jax.lax.rsqrt,
                out_kwarg=True,
                Torchish_member=True)
auto_implements(torch.sigmoid,
                jax.nn.sigmoid,
                out_kwarg=True,
                Torchish_member=True)
...
def arange(*args, **kwargs):
    dtype = t2j_dtype(
        kwargs.get(
            "dtype",
            torch.get_default_dtype() if
            (isinstance(args[0], float) or
             (len(args) > 1 and isinstance(args[1], float)) or
             (len(args) > 2 and isinstance(args[2], float))) else torch.int64,
        ))
...
@implements(
    torch.bernoulli, out_kwarg=True
)  # don't set member because Tensor.bernoulli has different signature
...
def empty(
    *args,
    ...
    return jnp.empty(_args_to_shape(args),
...
def max(input, *args, **kwargs):
    ...
    if "dim" in kwargs or (args and not isinstance(args[0], Torchish)):
        ...
        if not keepdim:
            values, indices = jnp.squeeze(values,
                                          axis=dim), jnp.squeeze(indices,
                                                                 axis=dim)
...
def multinomial(input, num_samples, replacement=False, generator=None):
    ...
    if input.ndim == 1:
        N = input.shape[0]
        return jax.random.choice(mk_rng(),
                                 N,
                                 shape=(num_samples, ),
                                 replace=replacement,
        ...
        return jax.vmap(
            lambda rng, w: jax.random.choice(rng,
                                             N,
                                             shape=(num_samples, ),
                                             replace=replacement,
                                             p=w / w.sum()),
...
def normal(*args, **kwargs):
    ...
    shape = kwargs.get(
        ...
        (mean.shape if isinstance(mean, jnp.ndarray) else
         (std.shape if isinstance(std, jnp.ndarray) else None)),
    )
...
def ones(*args,
         ...
         device=None,
         requires_grad=False):
    return jnp.ones(_args_to_shape(args),
                    dtype=t2j_dtype(dtype or torch.get_default_dtype()))
...
def rand_like(input,
              ...
              requires_grad=False,
              memory_format=torch.preserve_format):
    return jax.random.uniform(mk_rng(),
                              shape=input.shape,
...
def randint(*args, **kwargs):
    ...
    return jax.random.randint(mk_rng(),
                              shape=shape,
                              minval=low,
                              maxval=high,
                              dtype=t2j_dtype(kwargs.get("dtype",
                                                         torch.int64)))
...
def randn_like(input,
               ...
               requires_grad=False,
               memory_format=torch.preserve_format):
    return jax.random.normal(mk_rng(),
                             shape=input.shape,
                             dtype=t2j_dtype(dtype or input.dtype))
...
def sort(input, dim=-1, descending=False, stable=False):
    input = _v(input)
    sorted_indices = jnp.argsort(input,
                                 axis=dim,
                                 stable=stable,
...
def tensor(data,
           ...
           pin_memory=False):
    assert not requires_grad
    return jnp.array(data.value if isinstance(data, Torchish) else data,
                     dtype=(t2j_dtype(dtype) if dtype is not None else None))
...
def unbind(input, dim=0) -> Sequence[Torchish]:
    return tuple(
        Torchish(input.value[(slice(None), ) * dim + (i, )])
        for i in range(input.value.shape[dim]))
...
def zeros(*args,
          ...
          requires_grad=False):
    assert not requires_grad
    return jnp.zeros(_args_to_shape(args),
                     dtype=t2j_dtype(dtype or torch.get_default_dtype()))
...
def adaptive_avg_pool2d(input, output_size):
    assert output_size == 1 or output_size == (
        1, 1), "TODO: implement output_size != 1"
...
def batch_norm(input,
    ...
    if training:
        ...
        running_mean.value = jax.lax.stop_gradient(momentum * mean +
                                                   (1 - momentum) *
                                                   running_mean.value)
        running_var.value = jax.lax.stop_gradient(momentum * var +
                                                  (1 - momentum) *
                                                  running_var.value)
    ...
    res = (x - working_mean.reshape(newshape)
           ) * jax.lax.rsqrt(working_var.reshape(newshape) + eps)
...
def _deconv_output_length(input_length,
  ...
  """
    ...
    if output_padding is None:
        if padding == "VALID":
            length = input_length * stride + jax.lax.max(
                filter_size - stride, 0)
        elif padding == "SAME":
            length = input_length * stride
        else:
            length = (input_length -
                      1) * stride + filter_size - padding[0] - padding[1]
        ...
        length = (input_length -
                  1) * stride + filter_size - total_pad + output_padding
    ...
    if padding == "VALID":
        ...
        if input_size != expected_input_size:
            raise ValueError(
                ...
                f"parameters is {expected_input_size} which doesn't "
                f"match the actual input size {input_size}.")
        padding_needed = jax.lax.max(0, (input_size - 1) * stride +
...
def gradient_based_conv_transpose(
  ...
  """
    ...
    dn = jax.lax.conv_dimension_numbers(lhs.shape, rhs.shape,
                                        dimension_numbers)
    ...
    if isinstance(padding, str):
        if padding in {"SAME", "VALID"}:
            ...
            raise ValueError(
                f"`padding` must be 'VALID' or 'SAME'. Passed: {padding}.")

    inferred_output_shape = tuple(
        map(_deconv_output_length, i_sdims, k_sdims, padding, output_padding,
            strides, dilation))
    ...
    pads = tuple(
        map(_compute_adjusted_padding, i_sdims, output_shape, k_sdims, strides,
            padding, dilation))
    ...
    return jax.lax.conv_general_dilated(lhs,
                                        rhs,
                                        one,
                                        pads,
                                        strides,
...
def dropout(input, p=0.5, training=True, inplace=False):
    ...
    assert 0 <= p <= 1, "dropout probability has to be between 0 and 1, but got {}".format(
        p)
...
def embedding(input,
    ...
    if padding_idx is not None:
        weight = weight.at[padding_idx].set(
            jax.lax.stop_gradient(weight[padding_idx]))
    ...
    if scale_grad_by_freq:
        ...
        def f_jvp(primals, tangents):
            ...
            indices, counts = jnp.unique_counts(input,
                                                size=size,
...
def max_pool1d(input,
    ...
    return jax.lax.reduce_window(
        ...
        window_dimensions=(1, 1,
                           kernel_size) if isinstance(kernel_size, int) else
        (1, 1) + kernel_size,
        window_strides=(1, 1, stride) if isinstance(stride, int) else
        (1, 1) + stride,
        padding=[(0, 0), (0, 0),
                 (padding,
                  padding)] if isinstance(padding, int) else [(0, 0),
                                                              (0, 0), padding *
                                                              2],
    )
...
def max_pool2d(input,
    ...
    return jax.lax.reduce_window(
        ...
        window_dimensions=(1, 1, kernel_size,
                           kernel_size) if isinstance(kernel_size, int) else
        (1, 1) + kernel_size,
        window_strides=(1, 1, stride, stride) if isinstance(stride, int) else
        (1, 1) + stride,
...
def prelu(input: Torchish, weight: Torchish):
    ...
    if input.ndim == 0:
        ...
        weight = Torchish(
            jax.lax.broadcast_in_dim(_v(weight), input.shape,
                                     () if weight.ndim == 0 else
                                     (0 if input.ndim == 1 else 1, )))
...
def scaled_dot_product_attention(query,
    ...
    Q, K, V = jnp.swapaxes(Q, -2,
                           -3), jnp.swapaxes(K, -2,
                                             -3), jnp.swapaxes(V, -2, -3)
    mask, bias = None, None
    if attn_mask is not None:
        ...
        if jnp.issubdtype(attn_mask.dtype, jnp.bool_):
            ...
            raise ValueError(
                f"Unsupported attn_mask dtype: {attn_mask.dtype}. Expected bool or float."
            )
    output = jax.nn.dot_product_attention(Q,
                                          K,
                                          V,
                                          scale=scale,
                                          mask=mask,
...
@implements(torch.nn.functional.multi_head_attention_forward,
            Torchishify_output=False)
def multi_head_attention_forward(
    ...
    Q, K, V = jnp.swapaxes(Q, 0, 1), jnp.swapaxes(K, 0,
                                                  1), jnp.swapaxes(V, 0, 1)
    ...
    sdpa = jnp.concatenate(
        tuple(
            _v(
                scaled_dot_product_attention(Torchish(q), Torchish(k),
                                             Torchish(v)))
            for q, k, v in zip(
...
def t2j_dtype(dtype):
    return next(j_dtype for t_dtype, j_dtype in TJ_DTYPE_ASSOCIATION
                if t_dtype == dtype)


def j2t_dtype(dtype):
    return next(t_dtype for t_dtype, j_dtype in TJ_DTYPE_ASSOCIATION
                if j_dtype == dtype)
        ...
        def visit(m, prefix):
            for name, param in m.named_parameters(recurse=False):
                if param in reverse_dict:
                    ...
                    m._parameters[name] = Torchish(
                        state_dict[reverse_dict[param]])
                else:
                    m._parameters[name] = Torchish(
                        state_dict[".".join(prefix + [name])])
            ...
            for name, buffer in m.named_buffers(recurse=False):
                ...
                if ".".join(prefix + [name]) not in module.state_dict().keys():
                    ...
                    m._buffers[name] = Torchish(state_dict[".".join(prefix +
                                                                    [name])])
...
def j2t(thing):
    ...
    if isinstance(thing, jnp.dtype) or thing in [
            dt for _, dt in TJ_DTYPE_ASSOCIATION
    ]:
