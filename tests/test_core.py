...
def test_nan_to_num():
    for value in ["nan", "inf", "-inf"]:
        ...
        t2j_function_test(torch.nan_to_num, [(3, )],
                          samplers=samplers,
                          num_tests=1)
    ...
    t2j_function_test(torch.nan_to_num, [(3, )],
                      kwargs=dict(nan=0.0, posinf=1.0, neginf=-1.0),
                      samplers=samplers,
                      num_tests=1)
...
def test_get_set_item():
    ...
    t2j_function_test(lambda x: x[torch.tensor([1, 2]), :,
                                  torch.tensor([2, 3])], [(3, 4, 5)],
                      tests=tests)
    # when the slice object is dynamic, jax will refuse to run with jit.
    t2j_function_test(lambda x: x[1:, torch.tensor(1):torch.tensor(3), 2],
                      [(3, 4, 5)],
                      tests=tests_nojit)
    ...
    tests_nojit = [
        partial(forward_test, test_jit=False),
        partial(backward_test, argnums=(1, ))
    ]
    t2j_function_test(lambda x, y: f(x, [0, 1, 2], y), [(3, 4, 5), ()],
                      tests=tests)
    t2j_function_test(lambda x, y: f(x, [slice(None), 1, 2], y), [(3, 4, 5),
                                                                  (3, )],
                      tests=tests)
    t2j_function_test(lambda x, y: f(x, [0, slice(None), 2], y), [(3, 4, 5),
                                                                  (4, )],
                      tests=tests)
    t2j_function_test(lambda x, y: f(x, [0, 1, slice(None)], y), [(3, 4, 5),
                                                                  (5, )],
                      tests=tests)
    t2j_function_test(
        lambda x, y: f(x, [slice(1, None), slice(1, 3), 2], y), [(3, 4, 5),
                                                                 (2, 2)],
        tests=tests)
    t2j_function_test(
        lambda x, y: f(x, [slice(1, None), slice(1, 3), 2], y), [(3, 4, 5),
    ...
    t2j_function_test(
        lambda x, y: f(
            x, [slice(1, None),
                slice(torch.tensor(1), torch.tensor(3)), 2], y),
        [(3, 4, 5), (2, 2)],
        tests=tests_nojit,
    )
    t2j_function_test(
...
def test_oneliners():
    ...
    t2j_function_test(lambda x: 3 * torch.mean(x), [(5, )],
                      atol=1e-6,
                      tests=fb)
    ...
    t2j_function_test(torch.mean, [(5, 6)],
                      kwargs=dict(dim=1, keepdim=False),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.mean, [(5, 6)],
                      kwargs=dict(dim=1, keepdim=True),
                      atol=1e-6,
                      tests=fbmo)
    ...
    t2j_function_test(torch.sum, [(5, 6)],
                      kwargs=dict(dim=1, keepdim=False),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.sum, [(5, 6)],
    ...
    t2j_function_test(torch.all, [(3, 2)],
                      samplers=samplers,
                      kwargs=dict(dim=1),
                      tests=fmo)
    t2j_function_test(torch.any, [(3, 2)], samplers=samplers, tests=fmo)
    t2j_function_test(torch.any, [(3, 2)],
                      samplers=samplers,
                      kwargs=dict(dim=1),
                      tests=fmo)
    ...
    t2j_function_test(
        ...
        [(3, 2)],
        samplers=[
            lambda key, shape: random.randint(
                key, shape, minval=0, maxval=1024)
        ],
        tests=fmo,
    )
    t2j_function_test(torch.bitwise_not, [(3, 2)],
                      samplers=[random.bernoulli],
                      tests=fmo)
    # logical operations
    samplers = [random.bernoulli, random.bernoulli]
    t2j_function_test(torch.logical_and, [(3, 2), (3, 2)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_and, [(3, 2), (2)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_and, [(3, 2), (3, 1)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_or, [(3, 2), (3, 2)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_or, [(3, 2), (2)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_or, [(3, 2), (3, 1)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_xor, [(3, 2), (3, 2)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_xor, [(3, 2), (2)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_xor, [(3, 2), (3, 1)],
                      samplers=samplers,
                      tests=fmo)
    t2j_function_test(torch.logical_not, [(3, 2)],
                      samplers=[random.bernoulli],
                      tests=fmo)
    t2j_function_test(torch.logical_not, [(2)],
                      samplers=[random.bernoulli],
                      tests=fmo)
    t2j_function_test(torch.logical_not, [(3, 1)],
                      samplers=[random.bernoulli],
                      tests=fmo)
    # tensor operators
    t2j_function_test(lambda x, y: x & y, [(3, 1), (3, 1)],
                      samplers=samplers,
                      tests=f)
    t2j_function_test(lambda x, y: x | y, [(3, 1), (3, 1)],
                      samplers=samplers,
    ...
    t2j_function_test(lambda x, y: x ^ y, [(3, 1), (3, 1)],
                      samplers=samplers,
                      tests=f)
    t2j_function_test(lambda x: ~x, [(3, 1)],
                      samplers=[random.bernoulli],
                      tests=f)
    ...
    t2j_function_test(torch.max, [(3, 5)],
                      kwargs=dict(dim=1),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.max, [(3, 5)],
                      ...
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.max, [(3, 5, 7)],
    ...
    t2j_function_test(torch.max, [(3, 5, 7)],
                      ...
                      tests=fbmo)
    t2j_function_test(torch.max, [(3, 5, 7)],
                      kwargs=dict(dim=2, keepdim=True),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.max, [(3, 5, 7)],
    ...
    t2j_function_test(torch.max, [(3, 5, 7)],
                      ...
                      tests=fbmo)
    t2j_function_test(torch.max, [(3, 5, 7)],
                      kwargs=dict(dim=2, keepdim=False),
                      atol=1e-6,
                      tests=fbmo)
    # test with combination of args and kwargs
    t2j_function_test(
        lambda input, out=None: torch.max(input, 2, False, out=out),
        [(3, 5, 7)],
        atol=1e-6,
        tests=fbo)
    t2j_function_test(
        lambda input, out=None: torch.max(input, 2, keepdim=False, out=out),
        [(3, 5, 7)],
        atol=1e-6,
        tests=fbo)
    # max to compare two tensor, returns the element-wise max values
    t2j_function_test(torch.max, [(3, 5, 7), (3, 5, 7)], atol=1e-6, tests=fbm)
    t2j_function_test(torch.max, [(3, 5, 7), (5, 7)], atol=1e-6,
                      tests=fbm)  # broadcasting
    t2j_function_test(torch.max, [(3, 5, 7), (7)], atol=1e-6,
                      tests=fbm)  # broadcasting
    # test with kwargs
    t2j_function_test(
        lambda input, other, out=None: torch.max(input, other=other, out=out),
        [(3, 5, 7), (3, 5, 7)],
        atol=1e-6,
        tests=fbo)
    t2j_function_test(torch.sort, [(3, 5)],
                      kwargs=dict(dim=0),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.sort, [(3, 5)],
                      ...
                      tests=fbmo)
    t2j_function_test(torch.sort, [(3, 5)],
                      kwargs=dict(dim=0, descending=True),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.topk, [(7, 9)],
                      kwargs=dict(k=1, dim=0),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.topk, [(7, 9)],
                      kwargs=dict(k=2, dim=0),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.topk, [(7, 9)],
                      kwargs=dict(k=3, dim=0),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.topk, [(7, 9)],
                      kwargs=dict(k=1, dim=1),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.topk, [(7, 9)],
                      kwargs=dict(k=2, dim=1),
                      atol=1e-6,
                      tests=fbmo)
    t2j_function_test(torch.topk, [(7, 9)],
                      kwargs=dict(k=3, dim=1),
                      atol=1e-6,
                      tests=fbmo)

    # Seems like an innocent test, but this can cause segfaults when using dlpack in t2j_array
    t2j_function_test(lambda x: torch.tensor([3.0]) * torch.mean(x), [(5, )],
                      atol=1e-6,
                      tests=fb)

    t2j_function_test(lambda x: torch.mul(torch.tensor([3.0]), torch.mean(x)),
                      [(5, )],
                      atol=1e-6,
                      tests=fb)
    t2j_function_test(lambda x: torch.tensor([3]) * torch.mean(torch.sqrt(x)),
                      [(3, )],
                      tests=fb)
    ...
    t2j_function_test(lambda x: x.view(2, 2) @ x.view(2, 2), [(2, 2)],
                      rtol=1e-6,
                      tests=fb)
    ...
    t2j_function_test(lambda x, y, out=None: torch.cat((x, y), out=out),
                      [(2, 3), (5, 3)],
                      tests=fbo)
    t2j_function_test(lambda x, y, out=None: torch.cat(
        (x, y), dim=-1, out=out), [(2, 3), (2, 5)],
                      tests=fbo)

    t2j_function_test(torch.flatten, [(2, 3, 5)], tests=fbm)
    t2j_function_test(torch.flatten, [(2, 3, 5)],
                      kwargs=dict(start_dim=1),
                      tests=fbm)
    t2j_function_test(torch.flatten, [(2, 3, 5, 7)],
                      kwargs=dict(start_dim=2),
                      tests=fbm)
...
def test_inplace_Tensor_methods():
    ...
    aac(
        vmap(t2j(f))(jnp.array([1, 2, 3])), jnp.array([f(1.0),
                                                       f(2.0),
                                                       f(3.0)]))
    ...
    def f4(x):
        ...
        with torch.no_grad():
            return torch.nn.functional.relu(
                x, inplace=True) - torch.nn.functional.relu(y, inplace=True)
