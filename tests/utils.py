...
def out_kwarg_test(f, args, kwargs={}, **assert_kwargs):
    torch_kwargs = jax.tree.map(
        lambda x: j2t(x) if isinstance(x, jnp.dtype) else x, kwargs)
    ...
    assert all(
        jax.tree.leaves(jax.tree.map(lambda x, y: x is y, jax_out2, jax_out3)))
...
def Torchish_member_test(f, args, kwargs={}, **assert_kwargs):
    ...
    jax_out1, jax_out2, jax_out3 = t2j(
        lambda *args: torch_function(*args, name=f.__name__))(*args)
    aac(jax_out1, jax_out3, **assert_kwargs)
    if hasattr(torch.Tensor, f"{f.__name__}_"):
        jax_out1, jax_out2, jax_out3 = t2j(
            lambda *args: torch_function(*args, name=f"{f.__name__}_"))(*args)
...
def args_generator(shapes, samplers=None, rng=random.PRNGKey(123)):
    ...
    while True:
        ...
        args = [
            sampler(rng, shape=shape) for rng, shape, sampler in zip(
                random.split(rng1, n_inputs), shapes, samplers)
        ]
        yield args
...
def backward_test(f, args, kwargs={}, argnums=None, **assert_kwargs):
    ...
    f_ = lambda *args: torch.cat(
        list(
            map(lambda x: x.flatten(),
                torch_tree_leaves(f(*args, **torch_kwargs))))).mean()
