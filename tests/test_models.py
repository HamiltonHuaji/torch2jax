...
def test_mlp():
    for activation in [torch.nn.ReLU, torch.nn.Tanh, torch.nn.Sigmoid]:
        model = torch.nn.Sequential(torch.nn.Linear(2, 3, bias=False),
                                    activation(), torch.nn.Linear(3, 5))
        input_batch = random.normal(random.PRNGKey(123), (3, 2))
        params = {
            k: random.normal(random.PRNGKey(123), v.shape)
        ...
        # Test gradients
        jax_grad = grad(lambda p: (jaxified_module(input_batch, state_dict=p)**
                                   2).sum())(params)
...
@pytest.mark.skipif(not is_network_reachable(),
                    reason="Network is not reachable")
def test_torchvision_models_resnet18():
    ...
    for eval in [False, True]:
        ...
        res_jax_jit, sd_jax_jit = jit(jaxified_module,
                                      static_argnames=["return_state_dict"])(
                                          input_batch,
                                          state_dict={
                                              **parameters,
                                              **buffers
