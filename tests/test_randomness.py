...
def test_multinomial():
    ### 1D input
    aac(
        t2j(lambda w: torch.sort(
            torch.multinomial(w, w.shape[-1], replacement=False)).values)(
                jr.uniform(jrk(0), (10, )), rng=jrk(0)),
        jnp.arange(10),
    )

    # 1D input, replacement=True
    t2j(lambda w: torch.multinomial(w, 11, replacement=True))(jr.uniform(
        jrk(0), (10, )),
                                                              rng=jrk(0))

    # Test that replacement=False catches 11 draws from a 10-element tensor.
    with pytest.raises(ValueError):
        t2j(lambda w: torch.multinomial(w, 11, replacement=False))(jr.uniform(
            jrk(0), (10, )),
                                                                   rng=jrk(0))
...
def test_rand_like():
    t2j(lambda x: torch.abs(torch.rand_like(x).mean() - 0.5) < 1e-3)(
        jnp.zeros(1_000_000), rng=jrk(0))
    t2j(lambda x: torch.sum(torch.abs(torch.rand_like(x) >= 0)) == x.numel())(
        jnp.zeros(1000), rng=jrk(0))
    t2j(lambda x: torch.sum(torch.abs(torch.rand_like(x) < 1)) == x.numel())(
        jnp.zeros(1000), rng=jrk(0))
...
def test_randint():
    ...
    t2j(lambda: torch.abs(
        torch.mean(torch.randint(-10, 10, (1000, )) < 0) - 0.5) < 1e-2)(
            rng=jrk(0))
    t2j(lambda: torch.abs(
        torch.mean(torch.randint(-10, 10, (1000, )) > 0) - 0.5) < 1e-2)(
            rng=jrk(0))
    t2j(lambda: torch.abs(
        torch.mean(torch.randint(0, 10, (1000, )) == 0) - 0.1) < 1e-2)(
            rng=jrk(0))
    t2j(lambda: torch.abs(torch.mean(torch.randint(10, (1000, )) == 0) - 0.1) <
        1e-2)(rng=jrk(0))
...
def test_Tensor_bernoulli_():
    ...
    aac(
        t2j(lambda x: x.bernoulli_(0.0))(jnp.zeros(5), rng=jrk(0)),
        jnp.zeros(5))
    aac(
        t2j(lambda x: x.bernoulli_(1.0))(jnp.zeros(5), rng=jrk(0)),
        jnp.ones(5))

    # test torch.Tensor values for p
    aac(
        t2j(lambda x: x.bernoulli_(torch.tensor(0.0)))(jnp.zeros(5),
                                                       rng=jrk(0)),
        jnp.zeros(5))
    aac(
        t2j(lambda x: x.bernoulli_(torch.tensor(1.0)))(jnp.zeros(5),
                                                       rng=jrk(0)),
        jnp.ones(5))
